...
// Start test block
describe("YHJToken", function () {
  before(async function () {
    this.YHJToken = await ethers.getContractFactory("YHJToken");
  });

  beforeEach(async function () {
    this.yhjToken = await this.YHJToken.deploy();
    await this.yhjToken.deployed();

    this.decimals = await this.yhjToken.decimals();
    ...
    this.signerContract = this.yhjToken.connect(signers[1]);
  });

  // Test cases
  it("Creates a token with a name", async function () {
    expect(await this.yhjToken.name()).to.exist;
    // expect(await this.yhjToken.name()).to.equal('YHJToken');
  });

  it("Creates a token with a symbol", async function () {
    expect(await this.yhjToken.symbol()).to.exist;
    // expect(await this.yhjToken.symbol()).to.equal('YHJ');
  });

  it("Has a valid decimal", async function () {
    expect((await this.yhjToken.decimals()).toString()).to.equal("18");
  });

  it("Has a valid total supply", async function () {
    const expectedSupply = ethers.utils.parseUnits("1000000", this.decimals);
    expect((await this.yhjToken.totalSupply()).toString()).to.equal(
      expectedSupply
    );
  });

  it("Is able to query account balances", async function () {
    const ownerBalance = await this.yhjToken.balanceOf(this.ownerAddress);
    expect(await this.yhjToken.balanceOf(this.ownerAddress)).to.equal(
      ownerBalance
    );
  });
  ...
  it("Transfers the right amount of tokens to/from an account", async function () {
    ...
    await expect(
      this.yhjToken.transfer(this.recipientAddress, transferAmount)
  ...
  it("Allows for allowance approvals and queries", async function () {
    ...
    expect(
      await this.yhjToken.allowance(this.recipientAddress, this.ownerAddress)
    ).to.equal(
      ethers.utils.parseUnits(approveAmount.toString(), this.decimals)
    );
  });
  ...
  it("Emits an approval event with the right arguments", async function () {
    ...
    await expect(
      ...
      .to.emit(this.yhjToken, "Approval")
      .withArgs(
        this.recipientAddress,
        this.ownerAddress,
        ethers.utils.parseUnits(approveAmount.toString(), this.decimals)
  ...
  it("Allows allowance to be increased and queried", async function () {
    ...
    const previousAllowance = await this.yhjToken.allowance(
      this.recipientAddress,
      this.ownerAddress
    );
